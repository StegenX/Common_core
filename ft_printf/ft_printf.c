...
/*   Updated: 2024/11/16 09:47:22 by aagharbi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	check_format_hp(va_list args, const char *format)
{
	...
	if (format[i + 1] == 'X')
	{
		*(unsigned int *)hptr = va_arg(args, unsigned int);
		count += ft_put_hex_upper(*(unsigned int *)hptr);
	}
	else if (format[i + 1] == 'x')
	{
		*(unsigned int *)hptr = va_arg(args, unsigned int);
		count += ft_put_hex_lower(*(unsigned int *)hptr);
	}
